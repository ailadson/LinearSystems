...
class Vector {
  ...
  equalTo(vector){
    ...
    for (var i = 0; i < a.length; i += 1) {
      ...
      if ( diff > 0.0000000000001) return false;
    }
    return true;
  }
  ...
  crossProduct(vector){
    var x1 = Decimal(this.coordinates[0]);
    var y1 = Decimal(this.coordinates[1]);
    var z1 = Decimal(this.coordinates[2]);

    var x2 = Decimal(vector.coordinates[0]);
    var y2 = Decimal(vector.coordinates[1]);
    var z2 = Decimal(vector.coordinates[2]);

    var crossCoords = [
      y1.mul(z2).sub(y2.mul(z1)).toNumber(),
      (-1 * x1.mul(z2).sub(x2.mul(z1)).toNumber()),
      x1.mul(y2).sub(x2.mul(y1)).toNumber()
    ];
